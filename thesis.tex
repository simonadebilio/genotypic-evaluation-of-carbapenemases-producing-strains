...
The sensitivity and specificity of the terrains need to be carefully evaluated: the use of less specific terrains can delay the disclosure of results.

\subsection{Direct seeding on McConkey agar}
The method involves the seeding of the swab on a McConkey agar plate.
During the seeding half of the plate is covered  with the swab (usually a rectal swab), rotate of 90° and then drawn with a sterile loop. 
This allow to spread the bacteria all over the plate and to make them grow separately in pure colonies.
After the seeding a 10 $\mu g$ Meropenem disk is position in the denser sowed area.
The plate is incubate for 16-24 hours at $\sim$35°C..

During this work have been considered colonies with the typical Enterobacteriaceae morphology, that were grown inside the diameter of the inhibition zone.
This test makes it easy to recognise the suspect colonies, and to report the results in a short time. 

\subsection{Enrichment in liquid terrains added with a carbapenem and seeding on McConkey agar}

This method is suggested by the Centers for Disease Control and Prevention.
 
This method involves the seeding of the rectal swab in 5 ml of Trytic Soy Broth containing  a 10 $\mu g$ meropenem disk. This is followed by a 18 hours incubation at 35°C.
After this, a subculture is seeded on a McConkey agar plate and is incubated at room temperature for 24-48 hours.

This test has a good sensitivity and makes it easy to recognise the colonies, but it requires a higher operational level than the previous methods.
...
During this tests the microorganism is exposed to standard concentrations of different antibiotics.
The most commonly methods, used in the microbiology laboratories are based on diffusion (i.e. the Kirby Bauer method), or on dilution (this method can be automate).

The first method involves the placement of antibiotic disks on the plate, and the following evaluation of the diameter of their inhibition zone. 
The second method allows to evaluate the Minimal Inhibitory Concentration (i.e. is the lowest antibiotic concentration necessary to inhibit the bacterial growth) for specific antibiotics.

The diameter of the inhibition zone, as well as the MICs, are related to standard threshold values (i.e. breakpoints) for the different microorganism-antibiotic combinations.

The breakpoint results can be converted into three categories of interpretation:
\begin{itemize}
\item sensitive (S), when the bacteria is killed or inhibited by the antibiotic;
\item intermediate (I), when the bacteria need high concentrations of antibiotic to be inhibited;
\item resistant (R), when the bacteria grows in presence of the antibiotic.
\end{itemize}

\subsection{Synergy tests}
The synergy test is a method of combination that tests the strains resistance to carbapenemase in combination with a specific inhibitor for the carbapenemases:
\begin{itemize}
...
\item the phenylboronic acid (APB) is able to inhibit KPC enzymes, but it can give false positive results in the presence of AmpC;
\item cloxacillin (CLX) combined with phenylboronic acid highlights AmpC producing strains associated with porines loss;
\item temocillin highlights the high resistance level of OXA-48 strains.
\end{itemize}

During the test procedure some pure colonies are taken with a sterile loop and dilute in 3ml of sterile saline until a 0.5 McFarland concentration.
Then, a sample is taken from the solution containing the inoculum, and is sown on a Agar Muller Hinton (MHA) plate (this medium contains meat extract, casein acid hydrolysate, and soluble starch). Here, at a distance of 15-22 mm, are placed the antibiotic disks of (?):
\begin{itemize}
...
The plate is incubated for 18-24 hours in aerobic environment at 35°C.

After the incubation period the inhibition zones are measured: these are proportionate to the microorganism sensitivity to the specific antibiotic. 
In this way (?), the inhibition zone is proportionate to the minimum inhibitory concentration.
...
The procedure involves the use of VITEK® 2 GP or GN cards for the microbial identification (ID), and of VITEK® 2 AST cards, specific for the antibiogram. 

Every card shows a unique bar code that act as a link between the card and the patient number.

These cards have 64 wells (? in italiano si chiamano "pozzetti" ma non sono riuscita a trovare un corrispettivo se non questo) containing biochemical reagents or antibiotics. Every card has also a little tube that transfer the suspension into the wells.
...
Once the cassette is loaded the incubation ad reading of each card is done by the instrument.

Thanks to the development of fluorescent substrate in both cards (GP or GN, and AST) this instrument is able to identify the accurate microorganism and to give us (??) its precise MIC converted into one of the three categories of interpretation (S, I, or R). (??)

\subsubsection{GeneXpert®: recognition of genotypic variants}
The Cepheid Xpert® CARBA-R test is diagnostics qualitative analysis designed to rapidly detect and differentiate gene sequences that give resistance to carbapenem antibiotics (ordine parole?).

This test can identify the following genes: blaKPC, blaNDM, blaVIM, blaOXA-48, and blaIMP-1.

The Xpert® CARBA-R test performs a PCR (polymerase chain reaction) multiplex reaction of amplification, to detect specific DNA target sequences after a short sample preparation time.

Every instrument contains  a syringe for dispensing the sample, a ultrasonic horn for the cell lysis, and a thermal cycler for the PCR execution and the target detection.

During the sample preparation some pure colonies are dilute in saline solution until a 0,5 McFarland concentration. Then, $20 \mu l$ of this solution are added to a specific (pre-fornito? non mi viene il termine corretto in italia!) reagent. This second solution is shaked for 10 seconds with a vortex mixer, then put in a specific disposable cartridge.

The GeneXpert platform performs the subsequent (?) steps: a filter captures the sample bacterial cells, that are broken with ceramic marbles and a ultrasonic horn. 
The solution containing the DNA is combined with the PCR dried reagents, and transferred  into an optic tube were the real time PCR is done. (??)
